...
static worker_t *s_worker_new (unsigned heartbeat, zframe_t *identity)
{
    worker_t *self = (worker_t *) zmalloc (sizeof (worker_t));
    self->identity = identity;
    self->id_string = zframe_strhex (identity);
    self->expiry = zclock_time ()
                 + heartbeat * HEARTBEAT_LIVENESS;
    return self;
}

//  Destroy specified worker object, including identity frame.
void s_worker_destroy ( worker_t **self_p)
{
    assert (self_p);
    if (*self_p) {
...
{
    Sbqueue *mq = (Sbqueue*)malloc(sizeof(Sbqueue));
    if (!mq)
        return NULL;
    mq->front_port = fport;
    mq->back_port = bport;
    mq->heartbeat_msec = heartbeat;
    return mq;
}
int SbqueueStart(Sbqueue *mq)
{
    ...
    char url[20] = { 0 };
    snprintf(url, sizeof(url), "tcp://*:%d", mq->front_port);
    int rc = zmq_bind(frontend, url);    //  For clients
    assert(rc == 0);
    snprintf(url, sizeof(url), "tcp://*:%d", mq->back_port);
    rc = zmq_bind(backend, url);    //  For workers
    assert(rc == 0);
    ...
    unsigned heartbeat_msec = mq->heartbeat_msec;
    uint64_t heartbeat_at = zclock_time() + heartbeat_msec;
    ...
    while (true) {
        ...
        if (items [0].revents & ZMQ_POLLIN) {
            ...
            printf("recv msg from worker server.\n");
            //  Any sign of life from worker means it's ready
            zframe_t *identity = zmsg_unwrap(msg);
            worker_t *worker = s_worker_new(heartbeat_msec, identity);
        ...
        //  dead workers:
        if (zclock_time() >= (int64_t)heartbeat_at) {
            worker_t *worker = (worker_t *) zlist_first (workers);
            while (worker) {
            ...
            heartbeat_at = zclock_time () + heartbeat_msec;
        }
